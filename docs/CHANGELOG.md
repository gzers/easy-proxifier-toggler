...
#### 1. 托盘菜单增强
- **开机自启动开关**: 在托盘菜单中直接切换开机自启动，无需打开设置界面
  - 启用时菜单项前显示 ✓ 打勾标记
  - 未启用时只显示标题文字
  - 点击即可切换状态
  - 自动同步 Windows 注册表
...
- 分割线间距: 5px (上) + 10px (下)
- 切换按钮上边距: 15px
- 按钮圆角统一为 `CORNER_RADIUS_LARGE` (8px)
...
- **关键改进**:
  - 使用 `pack_propagate(False)` 控制容器尺寸
  - 状态行和徽章容器设置固定尺寸
...
- 📖 新增 `docs/TRAY_MENU_SWITCHES.md` - 托盘菜单开关功能测试指南
- 📖 更新 `README.md` - 版本号更新到 2.4.0
- 📖 更新 `CHANGELOG.md` - 详细记录本次更新内容

### 版本信息
- **版本号**: 2.4.0
- **发布日期**: 2026-01-19
- **主要更新**: 托盘菜单增强 + 状态面板UI优化
- **兼容性**: 向后兼容 v2.3.x
